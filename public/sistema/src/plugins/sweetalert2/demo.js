...
demo = {
    ...
    showSwal: function(type, id, from ){
        if(type == 'basic'){
        	swal({
            ...
            swal({
                    ...
                    confirmButtonClass: "btn btn-success btn-fill mx-2",
                    cancelButtonClass: "btn btn-danger btn-fill mx-2",
              buttonsStyling: false,
              html:
              `<input id="item${id}" type="hidden" value="${from}" data-id="${id}">`
                }).then(function(e) {
                  if (e.value) {
                    var newid = document.querySelector(`#item${id}`).dataset.id
                    var from = document.querySelector(`#item${id}`).value
                    $.ajaxSetup({
                      headers: {
                    ...
                    $.ajax({
                        type: 'DELETE',
                        url: '/painel/' + from +'/' + newid + '/remocao',
                        data: {
                            id: newid
                        },
                        dataType: 'json',
                        success: function (data) {
                            location.assign(`/painel/${from}`)
                        },
                        error: function (data) {
                            console.log('Erro ao remover o elemento', data );
                        }
                    });
                  }
                }, function(dismiss) {
